...
其实,小球与小球的碰撞检测并不难,难点是碰撞后,**两个小球的速度该怎么变化**.

这里更多的是物理知识,因此,为了方便实现这个效果,我已经封装好了一个函数,用来计算两个小球碰撞后,速度的变化和坐标的变化,如下:
...
```
注：
1. 上面的函数是用来计算碰撞后的速度变化和坐标变化,并不包含碰撞检测
